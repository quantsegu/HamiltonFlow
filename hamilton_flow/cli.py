...
def main() -> None:
    ...
    with duckdb.connect(
        args.database, read_only=args.database != ":memory:"
    ) as connection:
        session = HamiltonFlow(args.manifest, DuckDBClient(connection))
        if args.command == "catalog":
            result = session.catalog()
        else:
            request = request_from_dict(json.loads(Path(args.request).read_text()))
            if args.command == "explain":
                result = {"sql": session.explain(request).sql_statement.sql}
            else:
